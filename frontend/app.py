...
# ==========================================
# GLOBAL VERİ YÜKLEME (Genel İstatistikler İçin)
# ==========================================
@st.cache_data
def load_data():
    try:
...
# ==========================================
# 2. FASTAPI BAĞLANTISI (API REQUEST)
# ==========================================
def get_api_prediction(data_dict):
    ...
    try:
        ...
        st.error("🚨 Backend API'sine ulaşılamıyor! FastAPI sunucusunun çalıştığından emin olun.")
        return None
...
def main_dashboard():
    ...
    # SIDEBAR FİLTRELEME MANTIĞI
    st.sidebar.subheader("Filter By:")
    geo_filter = st.sidebar.multiselect("Geography", options=["France", "Germany", "Spain"],
    ...
    if not df_filtered.empty:
        ...
        if card_filter == "Yes":
            df_filtered = df_filtered[df_filtered['HasCrCard'] == 1]
        elif card_filter == "No":
            df_filtered = df_filtered[df_filtered['HasCrCard'] == 0]
    ...
    tab_overview, tab1, tab2, tab3 = st.tabs(
        ["📊 Overview", "👤 Single Customer Analysis", "🧪 What-If Simulator", "📂 Batch Analysis"])
    ...
    with tab_overview:
        if df_filtered.empty:
            ...
            with obj_col:
                st.markdown("""
                <div style='background:#fff8f0; border-left:4px solid #bb3f3f; padding:12px 16px; border-radius:4px; font-size:13px; line-height:1.6;'>
                <b style='color:#bb3f3f;'>Objective :</b> To identify the customer segments currently experiencing churn and formulate
                actionable recommendations tailored for these groups, with the goal of enhancing retention efforts.
                </div>
            ...
            with c1:
                ...
                fig_gen.add_trace(go.Pie(
                    ...
                    hole=0.75, marker_colors=['#bb3f3f', '#e0e0e0'], textinfo='percent', textposition='outside',
            ...
            with c6:
                ...
                st.markdown("<p style='font-size:13px; color:#bb3f3f; font-weight:bold;'>Coefficient by Driver Name (API)</p>", unsafe_allow_html=True)

                try:
                    info_res = requests.get("http://backend:8000/model_info")
                    if info_res.status_code == 200:
                        model_info = info_res.json()
                        feat_imp = pd.DataFrame({'Feature': model_info["features"], 'Importance': model_info["importances"]})
                    ...
                    st.warning("Backend API'sine ulaşılamadığı için sürücü analizi yüklenemedi.")
    ...
    with tab1:
        ...
        if st.button("🔍 Perform Risk Analysis", use_container_width=True):
            st.session_state.run_analysis = True

        if st.session_state.get('run_analysis', False):

            with st.spinner("AI Calculating via FastAPI, Please Wait..."):
                ...
                if result:
                    ...
                    with res_col1:
                        ...
                        st.metric(label="System Status", value="Analysis Completed")
                        
                        # SHAP ANALİZİ API'DEN GELİYOR
    ...
    with tab2:
        if st.session_state.current_customer is not None:
            ...
            if st.button("🔄 Simulate Change Scenario", type="primary"):
                sim_data = cust.copy()
    ...
    with tab3:
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Analyze and Prioritize Entire List", use_container_width=True):
